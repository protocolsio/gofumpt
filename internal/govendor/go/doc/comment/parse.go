...
func (p *Parser) Parse(text string) *Doc {
	...
	for _, s := range parseSpans(lines) {
		...
		switch s.kind {
		default:
			panic("github.com/protocolsio/protocols-gofumpt/internal/govendor/go/doc/comment: internal error: unknown span kind")
		case spanList:
...
Spans:
	for {
		...
		if watchdog--; watchdog < 0 {
			panic("github.com/protocolsio/protocols-gofumpt/internal/govendor/go/doc/comment: internal error: not making progress")
		}
